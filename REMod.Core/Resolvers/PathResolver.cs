...
namespace REMod.Core.Resolvers
{
    public class PathResolver
    {
        ...
        public static string LogPath =>
            Path.Combine(FolderTypeStruct.Data, FileTypeStruct.Log);
        public static string SettingsPath =>
            Path.Combine(FolderTypeStruct.Data, FileTypeStruct.Settings);

        public static string DataPath(GameType type) =>
            Path.Combine(FolderTypeStruct.Data, GameTypeResolver.Path(type));

        public static string ModPath(GameType type) =>
            Path.Combine(FolderTypeStruct.Mods, GameTypeResolver.Path(type));

        public static string DownloadPath(GameType type) =>
            Path.Combine(
                FolderTypeStruct.Downloads,
                GameTypeResolver.Path(type)
            );

        public static string IndexPath(GameType type) =>
            Path.Combine(
                FolderTypeStruct.Data,
                GameTypeResolver.Path(type),
                FileTypeStruct.Cache
            );

        public static string Folder(FolderType folder, GameType type) =>
            folder switch
            {
